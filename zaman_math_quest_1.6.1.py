...
class QuizSession:
    ...
    def start(self, topic, difficulty):
        self.selected_topic = topic
        self.selected_difficulty = difficulty
        self.current_questions = self.quiz_data[topic][difficulty]
        self.question_no = 0
        self.score = 0

    def current_question(self):
        return self.current_questions[self.question_no]
    ...
    def has_more_questions(self):
        return self.question_no + 1 < len(self.current_questions)
...
def random_easy_algebra():
    a = random.randint(1,5)
    b = random.randint(1,10)
    c = a * random.randint(2,6) + b
    equation = f"{a}x + {b} = {c}"
    answer = str((c - b) // a)
    return {"question": f"Solve for x: {equation}", "answer": answer}


#Dictionary storing questions.
quiz = {
    "Algebra": {
        "Easy": [random_easy_algebra() for _ in range(10)],
        "Medium": [
    ...
    "Geometry": {
        "Easy": [
            ...
            {"question": "Which of the following shapes is a polygon? Options:  Circle   Ellipse   Curve   Triangle. Type the number of the correct option.", "answer": "Triangle", "image": "geometry_easy_4.jpg"},
...
def check_answer():
    ...
    is_correct, correct_answer = quiz_game.submit_answer(user_answer)

    if is_correct:
        feedback_label.config(text="✅ Correct!", fg="green")
    else:
        feedback_label.config(text=f"❌ Incorrect. The answer was {correct_answer}", fg="red")

    score_label.config(text=f"Score: {quiz_game.score}/{quiz_game.total_questions()}")
    next_button.config(state="normal")

#Puts the user to next question.
def next_question():
    if quiz_game.has_more_questions():
        quiz_game.next()
        progress_bar["value"] = quiz_game.question_no
        show_question()
        question_num_label.config(text=f"Question {quiz_game.question_no + 1} of {quiz_game.total_questions()}")
        answer_entry.delete(0, tk.END)
        feedback_label.config(text="")
        next_button.config(state="disabled")
    else:
        complete_topic_label.config(text=f"Topic: {quiz_game.selected_topic}")
        show_frame(complete_level)
        complete_questions_label.config(text=f"Questions Answered: {quiz_game.total_questions()}")
        complete_score_label.config(text=f"Correct Answers: {quiz_game.score}/{quiz_game.total_questions()}")
        save_progress(quiz_game.selected_topic, quiz_game.selected_difficulty, quiz_game.score, quiz_game.total_questions())

#Show current question and image if there is one.
def show_question():
    question_data = quiz_game.current_question()
...
def start_game():
    ...
    quiz_game.start(topic, difficulty)

    progress_bar["maximum"] = quiz_game.total_questions()
    ...
    show_question()
    question_num_label.config(text=f"Question 1 of {quiz_game.total_questions()}")
    next_button.config(state="disabled")
    score_label.config(text=f"Score: 0/{quiz_game.total_questions()}")
...
#Root window.
def main():
    global root, topic_box, difficulty_box
    global topic_label, difficulty_label, question_num_label
    global question_label, image_label, answer_entry
    global feedback_label, next_button, score_label, progress_bar
    global complete_topic_label, complete_questions_label, complete_score_label, complete_level
    global quiz_frame
    ...
    progress_bar = ttk.Progressbar(quiz_frame, orient="horizontal", length=400, mode="determinate")
